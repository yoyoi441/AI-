...
do {
    ...
    check(result.events.count == 2, "CodexJSONLParser: model-specific quota line still contributes tokens")
    check(result.latestPrimaryWindow?.usedPercent == 30.0, "CodexJSONLParser: model-specific 0% cannot replace account-wide quota")
    check(result.latestPrimaryWindow?.windowMinutes == 10080, "CodexJSONLParser: account-wide window remains selected")
    check(result.latestSecondaryWindow == nil, "CodexJSONLParser: model-specific secondary window is ignored")
    check(result.latestWindowEventTimestamp == isoDate("2026-09-11T08:17:00.000Z"), "CodexJSONLParser: quota timestamp belongs to accepted family")
} catch {
    failures += 1
    print("FAIL CodexJSONLParser mixed quota test threw: \(error)")
}
